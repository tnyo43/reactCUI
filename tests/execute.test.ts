...
describe("Execute", () => {
  ...
  test("'cd xxx'は子にDirectoryEntry xxxがあるときのみ成功する", () => {
    ...
    expect(execute.execute("cd hoge").result).toEqual([
      "cd: no such file or directory: hoge",
    ]);
    expect(execute.execute("cd poem.txt").result).toEqual([
      "cd: not a directory: poem.txt",
    ]);
    execute.execute("cd ..");
  ...
  test("'ls'は子を表示する", () => {
    execute.execute("cd ..");
    expect(execute.execute("ls").result).toEqual([
      "memo.txt",
      "note.tex",
      "subdocuments",
    ]);
  ...
  test("'mkdir xxx'は子にEntry xxxがないのみ成功する", () => {
    expect(execute.execute("mkdir subdocuments").result).toEqual([
      "mkdir: subdocuments: File exists",
    ]);
  ...
  test("'cat xxx'はFileEntry xxxがあるときのみ成功する", () => {
    ...
    expect(execute.execute("cat note.tex").result).toEqual([
      "I wrote note at Noto hanto",
    ]);
    expect(execute.execute("cat xxxx").result).toEqual([
      "cat: no such file or directory: xxxx",
    ]);
  });
});
