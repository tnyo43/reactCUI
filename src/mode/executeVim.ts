...
export default class ExecuteVim implements Execute {
  ...
  private delete() {
    ...
    if (w === 0) {
      ...
      this._cursor[1]--;
      this._text[h] = this._text[h]
        .slice(0, w - 1)
        .concat(this._text[h].slice(w));
    }
  }
  ...
  set cursor(c: [number, number]) {
    ...
    this._cursor[1] = Math.min(
      this._text[this._cursor[0]].length,
      Math.max(0, c[1])
    );
  }
